    ...
    if device_type == DEVICE_TYPE_LIGHT:
        ...
        return vol.Schema({
            ...
            vol.Required("button_1_join"): vol.All(
                cv.positive_int,
                vol.Range(min=MIN_DIGITAL_JOIN, max=MAX_DIGITAL_JOIN-3)  # Ensure space for 4 sequential joins
            ),
            vol.Optional("light_join"): vol.All(
                cv.positive_int,
                vol.Range(min=MIN_ANALOG_JOIN, max=MAX_ANALOG_JOIN)
            ),
            vol.Required("button_count"): vol.All(
                cv.positive_int,
                vol.Range(min=2, max=4)  # Must have 2-4 buttons
            ),
